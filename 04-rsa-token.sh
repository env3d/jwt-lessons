...
B64_HEADER=$(echo -n "$HEADER" | base64 -w 0 | tr '+' '-' | tr '/' '_' | tr -d '=')
B64_PAYLOAD=$(echo -n "$PAYLOAD" | base64 -w 0 | tr '+' '-' | tr '/' '_' | tr -d '=')

# We now produce the RSA-SHA256 signature using our private key
